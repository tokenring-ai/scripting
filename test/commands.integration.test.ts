...
describe("Command Integration Tests", () => {
  let agent: ReturnType<typeof createMockAgent>["agent"];
  let context: any;
  ...
  describe("echo command integration", () => {
    it("should display interpolated text", async () => {
      ...
      const result = echoCmd.execute({ remainder: "Hello $name", agent } as any);

      expect(result).toBe("Hello Alice");
    });

    it("should handle missing variables gracefully", async () => {
      const result = echoCmd.execute({ remainder: "Hello $missing", agent } as any);

      expect(result).toBe("Hello ");
    });

    it("should handle empty input", async () => {
      const result = echoCmd.execute({ remainder: "", agent } as any);
  ...
  describe("var command integration", () => {
    ...
    it("should interpolate variables in values", async () => {
      ...
      const _result = await varSetCmd.execute({ remainder: '$full = "Hello $first"', agent } as any);
    ...
    it("should handle function calls in var assignments", async () => {
      ...
      const _result = await varSetCmd.execute({ remainder: '$result = testFunc("test")', agent } as any);
    ...
    it("should prevent name conflicts with lists", async () => {
      ...
      expect(varSetCmd.execute({ remainder: '$conflict = "value"', agent } as any)).rejects.toThrow("already exists as a list");
    });
  });

  describe("func command integration", () => {
    it("should define expression functions", async () => {
      const result = funcDefineExpr.execute({ remainder: 'greet($name) => "Hello, $name"', agent } as any);
    ...
    it("should define LLM functions", async () => {
      const result = funcDefineLlm.execute({ remainder: 'analyze($text) => "Analyze: $text"', agent } as any);
    ...
    it("should define JavaScript functions", async () => {
      const result = funcDefineJs.execute({ remainder: "double($x) { return $x * 2; }", agent } as any);
    ...
    it("should prevent reserved function names", async () => {
      expect(() => funcDefineExpr.execute({ remainder: 'if($x) => "value"', agent } as any)).toThrow("reserved");
    });
    ...
    it("should handle invalid syntax", async () => {
      expect(() => funcDefineExpr.execute({ remainder: "invalid syntax", agent } as any)).toThrow("Invalid syntax");
    });
  });
  ...
  describe("func list command integration", () => {
    it("should list all functions", async () => {
      ...
      const result = funcList.execute({ agent } as any);
  ...
  describe("list command integration", () => {
    ...
    it("should handle variables in lists", async () => {
      ...
      const _result = await listCmd.execute({ remainder: "@names = [$name1, $name2]", agent } as any);
    ...
    it("should prevent name conflicts with variables", async () => {
      ...
      expect(listCmd.execute({ remainder: '@conflict = ["item"]', agent } as any)).rejects.toThrow("already exists as a variable");
    });
  });
  ...
  describe("call command integration", () => {
    ...
    it("should handle function call errors", async () => {
      await expect(callCmd.execute({ remainder: "nonExistentFunc()", agent } as any)).rejects.toThrow("Error during function call");
    });
  });
  ...
  describe("for command integration", () => {
    ...
    it("should handle missing lists", async () => {
      expect(forCmd.execute({ remainder: "$item in @missing { /echo $item }", agent } as any)).rejects.toThrow("not found");
    });
