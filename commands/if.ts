...
const inputSchema = {
  ...
  remainder: {
    name: "expression",
    description:
      "If condition and blocks: $condition { commands } [else { commands }]",
    required: true,
  },
  ...
  execute: async ({
    ...
    if (!prefixMatch) {
      throw new CommandFailedError(
        "Invalid syntax. Use: /if $condition { commands } [else { commands }]",
      );
    }
    ...
    if (isTruthy) {
      ...
      if (elseMatch) {
        const elseBlock = extractBlock(
          remainder,
          thenBlock.endPos + elseMatch[0].length,
        );
        if (!elseBlock) {
